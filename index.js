document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleClick(index) {
        ...
        // Check for 18 points rule
        if (points[currentPlayer] === 18) {
            points[currentPlayer] -= 18;
            console.log(`Player ${currentPlayer + 1} had exactly 18 points. Subtracting 18 points.`);
            if (currentPlayer === 0) {
                player1PointsDisplay.innerHTML = `Player 1 Points: <span id="bigger">${points[0]}</span>`;
            } else {
                player2PointsDisplay.innerHTML = `Player 2 Points: <span id="bigger">${points[1]}</span>`;
